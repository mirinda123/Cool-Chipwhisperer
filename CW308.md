...
设置示波器的采集参数，在修改完参数后，无需重启示波器，只需运行一下代码块即可更改参数
...
def mainProcess(num_of_traces):
    ...
    for i in tnrange(num_of_traces):
        ...
        if adc2mVChBMax[0] == 32512 && adc2mVChBMax[-1] == 32512:
...
3. 如果示波器出现故障，则重启的方法是，重新插拔线缆，`restart kernel`,重新运行代码
4. `.S` 文件不要和.c文件同名，否则会被makefile删掉
5. `.S`一定要大写
6. 如果采集看不到触发的上升沿或者下降沿，可能是示波器采集范围太短，另一种可能是代码执行过程中出错。对于后者，找到代码出错点的方法是，在代码中的不同位置放置`trigger_high()`。如果仍然出现玄学错误，考虑把蓝色板子卸下重新插拔，或是参考[https://github.com/newaetech/chipwhisperer-jupyter/blob/master/demos/Debugging%20the%20CW308_STM32F3%20Using%20ChipWhisperer.ipynb] 调bug
